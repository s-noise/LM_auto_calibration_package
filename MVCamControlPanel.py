# addapted version of the MVCamControlPanel.py module in the mv_camera sub-package
# of the matrix_vision_camera python package version 0.1.0 from Andreas Mittelberger 
# 
# addapted for use with the LM_auto_calibration package
# to detect the inserted objective of a LM and set the pixel to um scale automatically
#
#
# changes to original version (before -> after) by Stefan Manuel Noisternig, 22.7.2019:
#
# 1)    self.magnification_combo = ui.create_combo_box_widget(items=[5, 10, 20, 50, 100], item_text_getter=item_text_getter)
#     ->   self.magnification_combo = ui.create_combo_box_widget(items=['auto', '5x', '10x', '20x', '50x', '100x']) 
# integer items -> string items & added 'auto' item
#
# 2)  in:  def magnification_changed(item):  added line: 
#        connect_camera.load_spatial_calibrations(self.__current_camera_settings)
# this line was commented (deactivated) before; it is needed to read & load the calibration from the .json calibration file 
# when the magnification or binning changes
#
# 3)  in:  def magnification_changed(item):  added 2 lines: 
#        self.__current_camera_settings.spatial_calibration_dict.update(active = item)
#        connect_camera.save_spatial_calibrations(self.__current_camera_settings)
# this lines writes the current selected magnification item to the .json calibration file
# the 2 Extensions matrix_vision_camera and LM_auto_calibration communicate by this line and the pixel scale is only set
# when the active item is the 'auto' item
...
class PanelDelegate:
    ...
    def connect_functions(self):
        ...
        def magnification_changed(item):
            ...
            self.__current_camera_settings.spatial_calibration_dict.update(active = item)
            connect_camera.save_spatial_calibrations(self.__current_camera_settings)
            #print(str(self.__current_camera_settings))
